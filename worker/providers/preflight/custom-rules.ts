 ...
 *
 * Batch 13: Double Space rules (1 rule) - OPTIONAL
...
export async function runCustomRules(
  ...
  // Batch 13: Double Space rules (optional)
  results.push(...checkDoubleSpaceRules($, rulesMap));
...
/**
 * Pattern to detect double spaces in HTML content.
 * Matches:
 * - &nbsp;&nbsp; (two non-breaking space entities)
 * - &nbsp;  followed by regular space
 * - regular space followed by &nbsp;
 */
const DOUBLE_SPACE_PATTERN = /&nbsp;&nbsp;|&nbsp; | &nbsp;/g;

/**
 * Check for double spaces in content:
 * - PREFLIGHT_CONTENT_DOUBLE_SPACES: Detect "&nbsp;&nbsp;", "&nbsp; ", " &nbsp;" patterns
 *
 * Commonly occurs from copy-pasting content from Word or other editors.
 * Excludes header, footer, pre, and code elements.
 *
 * This is an OPTIONAL rule (off by default per project)
 */
function checkDoubleSpaceRules(
  $: CheerioAPI,
  rulesMap: ReleaseRulesMap
): ResultItemToCreate[] {
  const results: ResultItemToCreate[] = [];
  ...
  // Remove header/footer regions
  $clone('header, footer, [role="banner"], [role="contentinfo"]').remove();

  // Remove pre and code elements (spacing is intentional there)
  $clone('pre, code').remove();

  // Get the remaining HTML content
  const contentHtml = $clone('body').html() || '';

  // Find all double-space matches
  ...
  while ((match = DOUBLE_SPACE_PATTERN.exec(contentHtml)) !== null) {
    // Get surrounding context (50 chars before and after)
    const start = Math.max(0, match.index - 50);
    const end = Math.min(contentHtml.length, match.index + match[0].length + 50);
    let context = contentHtml.substring(start, end);

    // Clean up context for readability (remove HTML tags, normalize whitespace)
    context = context
      .replace(/<[^>]+>/g, '') // Remove HTML tags
      .replace(/\s+/g, ' ')    // Normalize whitespace
      .trim();

    // Only add if context has meaningful content
    if (context.length > 0) {
      matches.push({
        pattern: match[0],
  ...
  if (matches.length > 0) {
    results.push(
      createFail(
        'PREFLIGHT_CONTENT_DOUBLE_SPACES',
        `Found ${matches.length} double-space occurrence(s) in content`,
        IssueSeverity.MEDIUM,
        rulesMap,
        {
          count: matches.length,
          examples: matches.slice(0, 10), // Limit to first 10 examples
        }
      )
    );
  } else {
    results.push(
      createPass(
        'PREFLIGHT_CONTENT_DOUBLE_SPACES',
        'No double spaces detected in content',
        {}
      )
    );
  }
